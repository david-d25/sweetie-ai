...
//        val uploadUrl = vkApiClient.docs().getMessagesUploadServer(vkGroupActor).execute().uploadUrl.toString()
...
//        tempFile.delete()
    ...
    fun uploadPhotoAttachments(toId: Long, images: List<ByteArray>): List<MessageAttachment> {
        log.info("Uploading ${images.size} images to $toId")
        val uploadUrl = vkApiClient.photos().getMessagesUploadServer(vkGroupActor).execute().uploadUrl.toString()
        val tempFiles = images.map {
            val tempFile = File.createTempFile("vk-image", ".jpg")
            tempFile.deleteOnExit()
            tempFile.writeBytes(it)
            tempFile
        }
        val response = vkApiClient.upload().photoMessage(uploadUrl, tempFiles.associateBy { it.name }).execute()
        val attachments = vkApiClient.photos().saveMessagesPhoto(vkGroupActor, response.photo).execute()
